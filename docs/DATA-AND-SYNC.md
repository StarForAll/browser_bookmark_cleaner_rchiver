...
- 当前仓库尚未定义真实的 `BookmarkNode` / `DraftState` 运行时模型
- 当前仓库尚未实现浏览器书签读取、浏览器写回、WebDAV 上传/恢复
- 当前仓库尚未实现 `chrome.storage.local` 持久化适配器
- 当前仓库只完成了壳层 UI、动作入口文案和后续能力的结构占位
...
- v1 目标物理存储使用 `chrome.storage.local`
...
说明：本节是目标撤销模型；当前代码仅在 UI 文案中保留了 `Ctrl+Z` 操作提示，还没有真实撤销实现。
