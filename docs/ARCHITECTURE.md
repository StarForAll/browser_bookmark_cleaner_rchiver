...
- `T04` 至 `T07A` 已落地规范化草稿图谱契约、本地持久化契约与适配器、浏览器书签读取导入、启动引导，以及图谱基础编辑与拖拽移动
- 当前页面已实现五区工作区壳层、启动状态映射、状态弹窗关闭/重开持久化、最新启动结果持久化、顶部动作按钮占位和统一中文文案集中管理
- 当前自动化测试已覆盖工程基线、manifest/页面入口、浏览器书签读取与导入、本地持久化、启动引导、图谱编辑与拖拽校验
- `src/features/`、`src/domain/`、`src/adapters/` 已进入真实业务实现，不再只是目录骨架
- 当前仍未落地浏览器覆盖写回 / 确认流、WebDAV 配置与版本链路、搜索与重复聚焦、`Ctrl+Z` 撤销历史、本地备份与撤销覆盖恢复
