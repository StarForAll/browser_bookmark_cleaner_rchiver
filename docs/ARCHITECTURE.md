...
- `T04` 至 `T09A` 已落地规范化草稿图谱契约、本地持久化契约与适配器、浏览器书签读取导入、启动引导、图谱基础编辑与拖拽移动、draft-only `Ctrl+Z`、搜索与重复聚焦、状态反馈与禁用态说明，以及浏览器覆盖 / 同步确认门禁
- 当前页面已实现五区工作区壳层、启动状态映射、状态弹窗关闭/重开持久化、最近 3 条状态历史持久化、顶部动作区统一中文文案集中管理，以及浏览器覆盖 / 同步动作的共享确认框
- 当前自动化测试已覆盖工程基线、manifest/页面入口、浏览器书签读取与导入、本地持久化、启动引导、draft-only 撤销、搜索与重复聚焦、状态历史与禁用态反馈、浏览器覆盖 / 同步确认门禁，以及图谱编辑与拖拽校验
- `src/features/`、`src/domain/`、`src/adapters/` 已进入真实业务实现，不再只是目录骨架
- 当前仍未落地浏览器覆盖 / 同步的真实备份与执行链路、WebDAV 配置与版本链路，以及本地备份与撤销覆盖恢复
