...
| 场景 | 操作 |
|------|------|
| 有当前 task | 读取 `$TASK_DIR/check.md`、任务目标/验收标准/设计文档、改动范围与风险点 |
| 无当前 task（手动调用） | 提示用户：需要先执行 `/trellis:check` 生成 `check.md`，或手动指定 task-dir；不跳过 review-gate 的后续判定流程 |
...
- 默认 reviewer 数：1；可通过 `--reviewer-count N` 扩展（最大 4）
- 协调者必须在同一回复中既写 `reviewer-commands-round-<N>.md` 文件，又直接打印可复制的 reviewer 命令，不得分两次输出
