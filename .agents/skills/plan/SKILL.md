...
`/trellis:plan` 的职责是任务拆解与任务图规划。默认应把后续工作拆成多个 Trellis task，而不是把主要执行单元长期堆叠在一个 `task_plan.md` 里。`task_plan.md` 在此阶段只承担总览、依赖关系、阶段门禁和任务图摘要，不再作为主要任务承载面。
...
生成 task 图和 `task_plan.md` 摘要时，除了“前置任务 / 阻塞任务 / 并行任务”字段，还必须补齐以下执行信息：
...
建议 `task_plan.md` 至少包含以下完整结构，但它只是 task 图摘要：
...
$TASK_DIR/
├── task_plan.md   ← task 图摘要 / 依赖与门禁总览
└── ...
```

## 下一步推荐

**当前状态**: task 图已明确；若保留 `task_plan.md`，它只作为总览与关系摘要。
